  ...
  */
  def playGame() = {
    ...
    if(flipInput == "3") {
      ...
      println("Error with flip count handling. Flipping 1 card at a time.")
    }
  ...
  */
  def handleInput(line: String) = {
    ...
    //handles moving a card by calling multiple functions to convert command to usable data
      ...
      //check if current card is king
      //check if stack to move to has a blank card on top
      //if it does, pop the card 
      //push all subsequent cards
...
//Got flipping 3 cards at a time working, and got king moving to empty space properly
